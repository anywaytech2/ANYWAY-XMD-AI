...
zokou({ nomCom: "dev", categorie: "General", reaction: "♥️" }, async (dest, zk, commandeOptions) => {
    ...
    const devs = [
      { nom: "anyway🍃tech", numero: "255678892560" },
      { nom: "King Coder", numero: "255678892560" },
...
zokou({ nomCom: "developer", categorie: "General", reaction: "🤝" }, async (dest, zk, commandeOptions) => {
    ...
    const devs = [
      { nom: "ANYWAY🍃tech", numero: "255678892560" },
      { nom: "King Coder", numero: "255761374656" },
      // Ajoute d'autres développeurs ici avec leur nom et numéro
    ];

    let message = "👋 *Welcome to Anyway xmd* here is the developer numbers:\n\n";
    for (const dev of devs) {
      message += `----------------\n• ${dev.nom} : https://wa.me/${dev.numero}\n`;
    }
  var lien = mybotpic()
